...
class _EventDetailScreenState extends State<EventDetailScreen> {
      ...
      onRefresh: !isInit
          ? () async {
              CommentModel comments =
                  await api.getEventComment(widget.event.info.threadId);
              if (mounted) {
                _hotComments = addToModel(comments.hotComments);
                _comments = addToModel(comments.comments);

                hotTitle = "精彩评论";
                title = "最新评论 ${comments.total != 0 ? comments.total : ''}";
                isInit = true;
              }
              setState(() {});
          ...
          : null,
      onLoad: () async {
        CommentModel comments = await api.getEventComment(
            widget.event.info.threadId,
            offset: 20 * _pageIndex);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: widget.event.type != 39
            ? _content()
            : Store.connect2<PlaySongModel, PlayVideoModel>(
