...
class _PlayListScreenState extends State<PlayListScreen> {
  ...
  Future showDesc(BuildContext context, PlayListDetailModel detail) {
  ...
  Widget content(PlayListDetailModel detail) {
    return CustomScrollView(controller: _controller, slivers: <Widget>[
      SliverAppBarCustom(
          ...
          background: Stack(
            children: <Widget>[
              FadeNetWorkImage(detail.playlist.coverImgUrl,
                  fit: BoxFit.fitWidth),
              BackdropFilter(
                filter: ImageFilter.blur(
                  sigmaY: widget.isOfficial ? 1 : 20,
                  sigmaX: widget.isOfficial ? 1 : 20,
                ),
                child: Container(
